  ...
  class MultipleSelect {
    ...
    initSelected(ignoreTrigger) {
      ...
      for (const row of this.data) {
        if (row.type === 'optgroup') {
          const selectedCount = row.children.filter(child => child.selected && !child.disabled && child.visible && !child.divider).length;
      ...
      const selectableVisibleCount = this.data.filter(row => !row.disabled && row.visible && !row.divider).length;
      const selectedVisibleCount = this.data.filter(row => row.selected && !row.disabled && row.visible && !row.divider).length;
      this.allSelected = selectableVisibleCount > 0 && selectedVisibleCount === selectableVisibleCount;
      if (!ignoreTrigger) {
        if (this.allSelected) {
    ...
    events() {
      ...
      this.$filterOptions.off('click').on('click', e => {
        ...
        this.$filterOptions.removeClass('active').attr('aria-pressed', 'false');
    ...
    filterBySelection(visible, row) {
      ...
      if (!visible || !this.options.filterOptions || this.currentFilter === 'all') {
        return visible;
      }
