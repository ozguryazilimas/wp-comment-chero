...
The head pass looks for the poll widget, a `[poll]` or `[page_polls]` shortcode and
the two blocks, and anything rendering after it — a template tag, a poll in a loop
page — asks for the assets as it renders and gets them from the footer.

Neither reaches poll markup fetched over `admin-ajax.php` or the REST API into a page
...
a "Loading ..." that never goes away.

Say so from the page that will hold the poll:

~~~
add_filter( 'wp_polls_needs_assets', '__return_true' );
~~~

Anywhere the poll can be predicted this early will do. Code that already runs later
in the page — a `the_content` filter inserting a placeholder, say — can call
